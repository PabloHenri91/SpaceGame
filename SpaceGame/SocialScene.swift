...
//            let con = Reachability.isConnectedToNetwork()
...
////                let label = (Label(name:"label\(i)", textureName:"\(name)", x:637, y:y+35, align:.center))
////                (label.childNodeWithName("label\(i)") as! SKLabelNode).horizontalAlignmentMode = SKLabelHorizontalAlignmentMode.Left
////                let switch1 = (Switch(name: "switch\(name)", textureName: "switchFavorite", x: 1106, y: y+8, align: .center))
...
//    
////    func buscaPlayers(){
////        let relation  = PFUser.currentUser()!.relationForKey("friends")//pegar favoritos
...
////        let switch1 = (self.childNodeWithName("switch\((user as! PFUser).username!)") as! Switch)
////        switch1.switchPressed()
